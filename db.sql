...
CREATE TABLE public.localized_videos (
  ...
  video_url text,
  thumbnail_url text,
  status character varying DEFAULT 'draft'::character varying CHECK (status::text = ANY (ARRAY['not-started'::character varying, 'queued'::character varying, 'processing'::character varying, 'draft'::character varying, 'live'::character varying, 'failed'::character varying]::text[])),
  duration integer,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.processing_jobs (
  ...
  status character varying DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying, 'processing'::character varying, 'completed'::character varying, 'failed'::character varying, 'waiting_approval'::character varying, 'ready'::character varying]::text[])),
