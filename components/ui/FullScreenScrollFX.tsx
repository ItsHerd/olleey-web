...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
    (
        ...
        const splitWords = (text: string) => {
            ...
            // Split by lines first to support manual \n breaks
            const lines = text.split('\n');
            return lines.map((line, lineIdx) => (
                <React.Fragment key={lineIdx}>
                    {line.split(/\s+/).filter(Boolean).map((w, i, arr) => (
        ...
        useLayoutEffect(() => {
            ...
            featuredRefs.current.forEach((el, sIdx) => {
                if (!el) return;
                gsap.set(el, {
                    y: sIdx === index ? 0 : (sIdx > index ? 40 : -40),
        ...
        const changeSection = (to: number) => {
            ...
            const outEl = featuredRefs.current[from];
            const inEl = featuredRefs.current[to];
            ...
            if (outEl) {
                gsap.to(outEl, {
                    ...
                    duration: D * 0.8,
                    ease: "power2.inOut",
                    onComplete: () => { gsap.set(outEl, { visibility: "hidden" }); }
                });
            }
            if (inEl) {
                gsap.set(inEl, { visibility: "visible", y: down ? 40 : -40, opacity: 0 });
                gsap.to(inEl, {
                    y: 0,
                    opacity: 1,
                    duration: D,
                ...
                // Animate words inside if present (using selector for robustness)
                const words = inEl.querySelectorAll('.fx-word');
                if (words.length) {
                    gsap.fromTo(words,
                        { yPercent: 100 },
                        { yPercent: 0, duration: D, stagger: 0.05, ease: "power3.out", delay: 0.1 }
                    );
                }
            }
        ...
        return (
                                        ...
                                        {sections.map((s, sIdx) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    ref={(el) => { if (el) featuredRefs.current[sIdx] = el; }}
                                                >
                                                    <h3 className="fx-featured-title">
                                                        {isString ? splitWords(s.title as string) : s.title}
                                                    </h3>
                                                    {s.description && (
                                                        <p className="fx-featured-desc">{s.description}</p>
                                                    )}
                                                </div>
                                            );
                                        })}
          ...
          .fx-fixed-section { height: ${Math.max(1, total + 1)}75vh; position: relative; }
          ...
          .fx-header {
            grid-column: 1 / 13; align-self: start; padding-top: 4vh;
          ...
          .fx-featured { 
            position: absolute; opacity: 0; visibility: hidden; will-change: transform, opacity;
            width: 100%; max-width: 80vw; margin: 0 auto;
          }
          ...
          .fx-featured-desc {
            margin: 1.5rem auto 0; color: var(--fx-text); opacity: 0.6;
            font-size: clamp(0.9rem, 1.2vw, 1.2rem); max-width: 400px;
            text-transform: none; font-weight: 500; line-height: 1.5;
          }
