...
export function NebulaCube() {
    ...
    // Create an enhanced star texture (client-side only)
  ...
  `;
    ...
    useEffect(() => {
        ...
        // Initialize star texture on client-side
        ...
        // Fade out cube as user scrolls, fully faded when entering #product section
        gsap.to(canvas, {
            scrollTrigger: {
                trigger: '#product',
                start: 'top bottom', // Start fading when #product enters bottom of viewport
                end: 'top top', // Fully faded when #product reaches top of viewport
                scrub: 1,
                onUpdate: (self) => {
                    if (canvasRef.current) {
                        canvasRef.current.style.opacity = String(1 - self.progress);
                    }
                }
            }
        });

        // Scroll animations for cube rotation
    ...
    return (
        <div ref={mountRef} style={{ width: '100vw', height: '100vh', position: 'relative' }}>
            <div className="content">
                {/* Empty content - just the 3D cube visual */}
            </div>
        </div>
    );
}
